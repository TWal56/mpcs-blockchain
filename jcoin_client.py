...
class TxnCast(jcoin_pb2_grpc.TxnCastServicer):
    ...
    def CastTrans(self, request, context):
        if request.transaction not in self.txn_mempool:
            txn_count = str(len(self.txn_mempool) + 1)
            self.txn_mempool[txn_count] = {'txn': request.transaction, 'from': request.addrMe}
        else:
            print('txn ' + request.transaction + ' is already known!')
...
def serve_txncast():
    ...
    server.add_insecure_port(socket.gethostbyname(socket.gethostname()) + ':58333')
    server.start()
    print('\nTxnCast Server is live on ' + node_ip + ':58333')
    server.wait_for_termination()
